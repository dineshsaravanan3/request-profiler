...
class Logger {
  ...
  logTextFormat(logData) {
    const { timestampLocal, method, path, status, durationMs } = logData;
    const time = timestampLocal || new Date(logData.timestamp).toLocaleTimeString();
    ...
    const isSlow = durationMs > (this.config.slowThreshold || 500);
